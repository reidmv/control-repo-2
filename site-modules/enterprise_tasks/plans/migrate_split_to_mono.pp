...
plan enterprise_tasks::migrate_split_to_mono {
  ...
  $master_status_hash = run_plan(enterprise_tasks::get_service_status, target => $master, service => 'puppet')
  $dbnode_status_hash = run_plan(enterprise_tasks::get_service_status, target => $dbnode, service => 'puppet')
  $result_or_error = catch_errors() || {
    ...
    run_task(enterprise_tasks::run_puppet, $master, max_timeout => 256)
    ...
    # Puppet agent runs
    run_task(enterprise_tasks::run_puppet, $console, max_timeout => 256)
    run_task(enterprise_tasks::run_puppet, $puppetdb, max_timeout => 256)
    run_task(enterprise_tasks::run_puppet, $master, max_timeout => 256)
  ...
  run_command("puppet resource service puppet ensure=${master_status_hash[status]} enable=${master_status_hash[enabled]}", $master)
  run_command("puppet resource service puppet ensure=${dbnode_status_hash[status]} enable=${dbnode_status_hash[enabled]}", $dbnode)
  if $result_or_error =~ Error {
    return fail_plan($result_or_error)
  } else {
    notice("${master} is now set up as a monolithic master, with a separate PE-PostgreSQL database node at ${dbnode}.")
  }
}
